...
cd $DOWNLOAD_DIR
if [ ! -f "rtmpdump-2.4.tar.gz" ]; then 
  git clone https://github.com/JudgeZarbi/RTMPDump-OpenSSL-1.1.git
  mv RTMPDump-OpenSSL-1.1 rtmpdump-2.4
  tar cvfz rtmp-2.4.tar.gz rtmpdump-2.4 
  rm -rf rtmpdump-2.4
  cd $BUILD_DIR
  tar --skip-old-files -xzvf $DOWNLOAD_DIR/rtmpdump-2.4.tar.gz
fi 
...
if [ ! -f "libxml2.tar.gz" ]; then 
  ...
  rm -rf libxml2
  cd $BUILD_DIR
  tar --skip-old-files -xzvf $DOWNLOAD_DIR/libxml2.tar.gz  
fi

cd $DOWNLOAD_DIR
if [ ! -f "ffmpeg-master.tar.gz" ]; then 
  git clone https://github.com/FFmpeg/FFmpeg.git
  tar cvfz ffmpeg-master.tar.gz FFmpeg 
  rm -rf FFmpeg
  cd $BUILD_DIR
  tar --skip-old-files -xzvf $DOWNLOAD_DIR/ffmpeg-master.tar.gz  
fi 

cd $DOWNLOAD_DIR
if [ ! -f "srt-master.tar.gz" ]; then 
  git clone --depth 1 https://github.com/Haivision/srt.git
  tar cvfz srt-master.tar.gz srt 
  rm -rf srt
  cd $BUILD_DIR
  tar --skip-old-files -xzvf $DOWNLOAD_DIR/srt-master.tar.gz
fi 
...
# tar --skip-old-files -xzvf /ffmpeg-static/dl/openssl-1.1.1g.tar.gz
...
cd $BUILD_DIR/srt*
[ $rebuild -eq 1 -a -f Makefile ] && make distclean || true
#PATH="$BIN_DIR:$PATH" cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="$TARGET_DIR"  -DENABLE_STATIC=ON -DENABLE_LOGGING=1 -DENABLE_PROFILE=1
...
# !!! missing libxml2 !!!!
# ./autogen.sh && ./configure --prefix=$TARGET_DIR && make && make install
# srt installed with cmake . -DCMAKE_INSTALL_PREFIX=/ffmpeg-static/target -DENABLE_APPS=1 -DENABLE_CODE_COVERAGE=1 -DENABLE_CXX_DEPS=1  -DENABLE_PKTINFO=1 -DENABLE_PROFILE=1 -DUSE_STATIC_LIBSTDCXX=1 -DENABLE_RELATIVE_LIBPATH=1 
## cmake . -DCMAKE_INSTALL_PREFIX=/ffmpeg-static/target -DENABLE_APPS=1 -DENABLE_CODE_COVERAGE=1  -DENABLE_PKTINFO=1 -DENABLE_PROFILE=1 -DUSE_STATIC_LIBSTDCXX=0 -DENABLE_CXX_DEPS=1

# patch srt.pc  @ /ffmpeg-static/target/lib/pkgconfig/srt.pc as following:
# libdir=${exec_prefix}/lib
# Libs: -L${libdir} -lsrt -lstdc++ -lm
# Remove -lgcc_s option
# pkg-config --libs srt
# -L/ffmpeg-static/target/lib -lsrt -lstdc++ -lm
...
# PKG_CONFIG_PATH=/ffmpeg-static/target/lib/pkgconfig
# TARGET_DIR=/ffmpeg-static/target
# BIN_DIR=/ffmpeg-static/bin
#./configure   --prefix="$TARGET_DIR"     --pkg-config-flags="--static"     --extra-cflags="-I$TARGET_DIR/include"     --extra-ldflags="-L$TARGET_DIR/lib"     --extra-libs="-lpthread -lm -lz"     --extra-ldexeflags="-static"     --bindir="$BIN_DIR"     --enable-pic     --enable-ffplay     --enable-fontconfig     --enable-frei0r     --enable-gpl     --enable-version3     --enable-libass     --enable-libfribidi     --enable-libfdk-aac     --enable-libfreetype     --enable-libmp3lame     --enable-libopencore-amrnb     --enable-libopencore-amrwb     --enable-libopenjpeg     --enable-libopus     --enable-librtmp     --enable-libsoxr     --enable-libspeex     --enable-libtheora     --enable-libvidstab     --enable-libvo-amrwbenc     --enable-libvorbis     --enable-libvpx     --enable-libwebp     --enable-libx264     --enable-libx265     --enable-libxvid     --enable-libzimg     --enable-nonfree     --enable-libsrt      --enable-zlib   --enable-libxml2  --enable-demuxer=dash     --enable-postproc     --enable-openssl
